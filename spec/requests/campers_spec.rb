  ...
  describe "GET /campers" do
    it "returns an array of all campers" do
      ...
      expect(response.body).to include_json(
        [
          { id: a_kind_of(Integer), name: "Caitlin", age: 8 },
          { id: a_kind_of(Integer), name: "Lizzie", age: 9 }
        ]
      )
    end
  ...
  describe "GET /campers/:id" do
        ...
        expect(response.body).to include_json(
          {
            ...
            age: 8,
            activities: [
              { id: a_kind_of(Integer), name: "Archery", difficulty: 2 },
              { id: a_kind_of(Integer), name: "Swimming", difficulty: 3 }
            ]
          }
        )
      end
    ...
    context "with an invalid ID" do
      it "returns an error message" do
        get "/campers/bad_id"
        expect(response.body).to include_json({ error: "Camper not found" })
      end
  ...
  describe "POST /campers" do
    context "with valid data" do
      ...
      it "creates a new Camper" do
        expect { post "/campers", params: camper_params }.to change(
          Camper,
          :count
        ).by(1)
      end
      ...
      it "returns the associated Camper data" do
        ...
        expect(response.body).to include_json(
          { id: a_kind_of(Integer), name: "Zoe", age: 11 }
        )
      end
