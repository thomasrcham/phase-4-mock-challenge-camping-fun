  ...
  describe "POST /signups" do
    context "with valid data" do
      let!(:signup_params) do
        { camper_id: Camper.first.id, activity_id: Activity.first.id, time: 10 }
      end

      it "creates a new Signup" do
        expect { post "/signups", params: signup_params }.to change(
          Signup,
          :count
        ).by(1)
      end
    ...
    context "with invalid data" do
      let!(:signup_params) do
        { camper_id: Camper.first.id, activity_id: Activity.first.id, time: 30 }
      end
